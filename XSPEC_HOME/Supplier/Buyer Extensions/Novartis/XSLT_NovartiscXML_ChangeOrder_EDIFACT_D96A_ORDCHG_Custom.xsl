...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Extension Start  -->
	<!-- Header Extrinsic -->
	<xsl:template match="//M_ORDCHG/S_FTX[D_4451 = 'ZZZ'][C_C108/D_4440='Terms and Condition']"/>

	<xsl:template match="//M_ORDCHG/G_SG26[1]">
		<xsl:if
			test="//cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'Terms and Condition']">
			<xsl:call-template name="FTXSG25">
	...
	<xsl:template name="FTXSG25">
		<xsl:param name="FTXData"/>		
		<xsl:if test="string-length($FTXData) &gt; 0">
			<xsl:choose>
				<!-- if string length is more than 1750 create 5 FTX in one sg25-->
				...
				<!-- if string length is less than 1750 create less than or equal to 5 FTX in one Sg25-->
				<xsl:otherwise>
					<xsl:variable name="temp">
