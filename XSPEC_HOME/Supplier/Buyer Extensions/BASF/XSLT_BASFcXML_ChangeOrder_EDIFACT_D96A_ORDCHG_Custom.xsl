...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="//M_ORDCHG/S_FTX[D_4451 = 'AAI'][1]">
		<xsl:choose>
	...
	<xsl:template match="//M_ORDCHG/S_FTX[D_4451 = 'AAI'][2]">
		<xsl:choose>
			<xsl:when test="//OrderRequestHeader/Comments[1]/@type != ''">
			</xsl:when>
			<xsl:otherwise>				
	...
	<xsl:template match="//M_ORDCHG/G_SG26/S_FTX[D_4451 = 'AAI'][1]">
		<xsl:variable name="lineNum" select="count(../preceding-sibling::G_SG26) + 1"/>
		<xsl:choose>
	...
	<xsl:template match="//M_ORDCHG/G_SG26/S_FTX[D_4451 = 'AAI'][2]">
		<xsl:variable name="lineNum" select="count(../preceding-sibling::G_SG26) + 1"/>
		<xsl:choose>
			<xsl:when test="//OrderRequest/ItemOut[$lineNum]/Comments[1]/@type != ''">
			</xsl:when>
	...
	<xsl:template match="G_SG3[last()]">
		<xsl:copy-of select="."/>
		<xsl:if test="not(exists(//G_SG3[S_NAD/D_3035='SU'])) and normalize-space(//OrderRequestHeader/Extrinsic[@name='partyAdditionalID'])!=''">
			<G_SG3>
				<S_NAD>
					<D_3035>SU</D_3035>
				</S_NAD>
