...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- Header Extensions -->
	<xsl:template match="//M_ORDCHG/S_FTX[D_4451 = 'ZZZ'][C_C108/D_4440 = 'purchaseConditions']">
		<xsl:choose>
	...
	<xsl:template match="//M_ORDCHG/S_FTX[C_C108/D_4440 != 'purchaseConditions'][last()]">
		<xsl:copy-of select="."/>
	...
	<xsl:template match="//M_ORDCHG/G_SG3[S_NAD/D_3035 = 'BT']">
		<xsl:element name="G_SG3">
	...
	<xsl:template match="//M_ORDCHG/G_SG10[last()]">
		<xsl:copy-of select="."/>
		<xsl:call-template name="createSG12"/>
	</xsl:template>
	<xsl:template match="//M_ORDCHG/G_SG9[not(exists(../G_SG10))][last()]">
		<xsl:copy-of select="."/>
		<xsl:call-template name="createSG12"/>
	</xsl:template>
	<xsl:template match="//M_ORDCHG/G_SG8[not(exists(../G_SG9))][last()]">
		<xsl:copy-of select="."/>
		<xsl:call-template name="createSG12"/>
	</xsl:template>

	<!-- Line Item Extensions -->
	<xsl:template match="//G_SG26/G_SG37[last()]">
		<!--  Added index to avoid duplicates -->
		<xsl:copy-of select="."/>
