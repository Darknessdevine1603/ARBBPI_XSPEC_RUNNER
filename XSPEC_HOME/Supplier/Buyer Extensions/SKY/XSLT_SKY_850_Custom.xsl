...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="//M_850/G_PO1/S_REF[D_127 = 'BuyerPartID1']">        
    </xsl:template>
    
    <xsl:template match="//M_850/G_PO1/S_PO1">        
        <xsl:variable name="lineNum" select="D_350"/>        
        <xsl:choose>
            <xsl:when test="//cXML/Request/OrderRequest/ItemOut[@lineNumber=$lineNum]/ItemDetail/Extrinsic[@name='BuyerPartID1']!=''">
                <S_PO1>					
                    <xsl:call-template name="AddItemPart">
                        <xsl:with-param name="PO1Segment" select="."/>
                        <xsl:with-param name="newPartQual" select="'BP'"/>
                        <xsl:with-param name="newPartValue" select="(//cXML/Request/OrderRequest/ItemOut[@lineNumber=$lineNum]/ItemDetail/Extrinsic[@name='BuyerPartID1'])"/>
                    </xsl:call-template>
                </S_PO1>
            </xsl:when>
            <xsl:otherwise>
