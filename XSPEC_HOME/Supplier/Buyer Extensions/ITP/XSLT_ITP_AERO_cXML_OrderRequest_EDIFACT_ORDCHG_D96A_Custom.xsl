...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- Extension Start  -->
  <!-- Header Extrinsic -->
  <xsl:template match="//M_ORDCHG/G_SG1[last()]">
    <xsl:copy-of select="."/>
    <xsl:if
  ...
  <xsl:template match="//M_ORDCHG/G_SG26/S_DTM[last()]">
  ...
  <xsl:template match="//M_ORDCHG/G_SG26/G_SG27[last()]">
    <xsl:variable name="lineNumber" select="../S_LIN/D_1082"/>
          ...
          <S_CCI>
            <C_C240>
              <D_7037>
                <xsl:value-of
                  select="
                    substring(normalize-space(/Combined/source/cXML/Request/OrderRequest/ItemOut[@lineNumber
