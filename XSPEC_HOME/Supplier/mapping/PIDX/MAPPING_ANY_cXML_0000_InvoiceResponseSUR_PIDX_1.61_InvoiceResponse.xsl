...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:pidx="http://www.pidx.org/schemas/v1.61" xmlns="http://www.w3.org/2001/XMLSchema" version="1.0">
	...
	<!-- Receiver DUNS -->
	<xsl:param name="anOrginalDocumentID"/>
		...
		<xsl:element name="pidx:InvoiceResponse">
			...
			<xsl:element name="pidx:InvoiceResponseProperties">
				<xsl:element name="pidx:InvoiceResponseNumber">
					<xsl:choose>
						...
						<xsl:when test="$anOrginalDocumentID != ''">
							<xsl:value-of select="$anOrginalDocumentID"/>
						</xsl:when>
						<xsl:otherwise>
				...
				<xsl:element name="pidx:PartnerInformation">
					...
					<xsl:choose>
						<xsl:when test="normalize-space(/cXML/Header/From/Credential[@domain = 'DUNS']/Identity) != ''">
							<xsl:element name="pidx:PartnerIdentifier">
								<xsl:attribute name="partnerIdentifierIndicator">DUNSNumber</xsl:attribute>
								<xsl:value-of select="substring(normalize-space(/cXML/Header/From/Credential[@domain = 'DUNS']/Identity), 1, 9)"/>
							</xsl:element>
			...
			<xsl:element name="pidx:InvoiceResponseDetails">
				<xsl:element name="pidx:InvoiceResponseLineItem">
					...
					<xsl:element name="pidx:StatusCode">
						<xsl:variable name="invStatusLine">
							<xsl:choose>
								<xsl:when test="cXML/Request/StatusUpdateRequest/DocumentStatus/@type != ''">
									<xsl:value-of select="cXML/Request/StatusUpdateRequest/DocumentStatus/@type"/>
								</xsl:when>
						...
						<xsl:choose>
							<xsl:when test="$cXMLPIDXLookupList/Lookups/InvoiceResponseCodes/InvoiceResponseCode[CXMLCode = $invStatusLine]/PIDXCode != ''">
								<xsl:value-of select="$cXMLPIDXLookupList/Lookups/InvoiceResponseCodes/InvoiceResponseCode[CXMLCode = $invStatusLine]/PIDXCode"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:text>Pending</xsl:text>
							</xsl:otherwise>
					...
					<xsl:element name="pidx:InvoiceResponseReason">
						<xsl:element name="pidx:ResponseReasonCode">
							<xsl:text>Other</xsl:text>
						</xsl:element>
						<xsl:element name="pidx:ResponseReasonComments">
							<xsl:text>See InvoiceResponseProperties\Comment for details</xsl:text>
						</xsl:element>
					</xsl:element>
				</xsl:element>
			</xsl:element>
