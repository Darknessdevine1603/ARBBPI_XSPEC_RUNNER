...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:pidx="http://www.pidx.org/schemas/v1.61"
    ...
    <!--<xsl:include href="Format_cXML_to_PIDX.xsl"/><xsl:variable name="cXMLPIDXLookupList" select="document('cXML_PIDXLookups.xml')"/>-->
    ...
    <!-- Receiver DUNS -->
    <xsl:template match="/">
        <xsl:element name="pidx:OrderChange">
            <xsl:attribute name="pidx:transactionPurposeIndicator">
                <xsl:choose>
                    <xsl:when test="normalize-space(cXML/Request/OrderRequest/OrderRequestHeader/@type) = 'update'">
            ...
            <xsl:element name="pidx:OrderChangeProperties">
                <xsl:element name="pidx:OrderChangeNumber">
                    <xsl:value-of select="cXML/Request/OrderRequest/OrderRequestHeader/@orderID"/>
                </xsl:element>
                <xsl:element name="pidx:OrderChangeDate">
                    <xsl:value-of select="substring-before(cXML/Request/OrderRequest/OrderRequestHeader/@orderDate, 'T')"/>
                </xsl:element>
                <xsl:element name="pidx:PurchaseOrderInformation">
                ...
                <xsl:if test="cXML/Request/OrderRequest/OrderRequestHeader/@orderVersion != ''">
                    <xsl:element name="pidx:SequenceNumber">
            ...
            <xsl:element name="pidx:OrderChangeDetails">
                        ...
                        <xsl:for-each select="cXML/Request/OrderRequest/ItemOut[not(@parentLineNumber != '')]">
                            <xsl:choose>
                                <xsl:when test="@itemType = 'composite'">
                                    <xsl:element name="pidx:OrderChangeLineItem">
            ...
            <xsl:element name="pidx:OrderChangeSummary">
    ...
    <xsl:template name="createlineitems">
            ...
            <xsl:when test="$isSubLine = 'true'">
                <xsl:element name="pidx:LineItemInformation">
                    ...
                    <xsl:if test="$item//ManufacturerPartID != ''">
                        <xsl:element name="pidx:LineItemIdentifier">
                            ...
                            <xsl:value-of select="$item//ManufacturerPartID"/>
                        </xsl:element>
                    </xsl:if>
                    <xsl:choose>
                ...
                <xsl:choose>
                    <xsl:when test="/cXML/Request/OrderRequest/OrderRequestHeader/@type = 'delete'">
                        <xsl:element name="pidx:ItemDetailChangeCode">
                            <xsl:text>Delete</xsl:text>
                        </xsl:element>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:element name="pidx:ItemDetailChangeCode">
                            <xsl:text>ModifyOther</xsl:text>
                        </xsl:element>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
        ...
        <xsl:if test="$createSubLine = 'true'">
            ...
            <xsl:for-each select="/cXML/Request/OrderRequest/ItemOut[@parentLineNumber = $parentLine]">
                <xsl:element name="pidx:OrderChangeSubLineItem">
                    <xsl:call-template name="createlineitems">
                        <xsl:with-param name="item" select="."/>
    ...
    <xsl:template name="UOMCodeConversion">
        ...
        <xsl:choose>
            <xsl:when test="lower-case($anANSILookupFlag) = 'true' and $cXMLPIDXLookupList/Lookups/UOMCodes/UOMCode[CXMLCode = $UOMCode]">
