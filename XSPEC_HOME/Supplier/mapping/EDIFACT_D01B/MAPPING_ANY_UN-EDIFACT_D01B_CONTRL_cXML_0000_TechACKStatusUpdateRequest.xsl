		...
		<cXML>
			...
			<Request>
				...
				<xsl:variable name="errorNotes">
					<xsl:choose>
						<xsl:when test="$ErrorCodeLookup/Lookups/ContrlErrorMsgs/ContrlErrorMsg[ErrorCode = $errorCode]/ErrorDescription">
							<xsl:value-of select="$ErrorCodeLookup/Lookups/ContrlErrorMsgs/ContrlErrorMsg[ErrorCode = $errorCode]/ErrorDescription"/>
						</xsl:when>
					</xsl:choose>
				</xsl:variable>
				...
				<StatusUpdateRequest>
					...
					<Status>
						<xsl:attribute name="text">
							<xsl:choose>
								<xsl:when test="M_CONTRL/S_UCI/D_0083 = '7'">Received</xsl:when>
								<xsl:otherwise>Failed</xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
						<xsl:attribute name="code">
							<xsl:choose>
								<xsl:when test="M_CONTRL/S_UCI/D_0083 = '7'">201</xsl:when>
								<xsl:otherwise>400</xsl:otherwise>
							</xsl:choose>
						</xsl:attribute>
						<xsl:choose>
							<xsl:when test="M_CONTRL/S_UCI/D_0083 = '7'">
								<xsl:text>Positive CONTRL received.</xsl:text>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="normalize-space(concat('Negative CONTRL received. ', $errorNotes))"/>
							</xsl:otherwise>
						</xsl:choose>
					</Status>
					<IntegrationStatus>
						<xsl:attribute name="documentStatus">
							<xsl:choose>
								<xsl:when test="M_CONTRL/S_UCI/D_0083 = '7'">customerReceived</xsl:when>
