			...
			<ApplicationArea>
				<Sender>
					...
					<TaskID>
						<xsl:choose>
							<xsl:when
								test="($isChange = 'update' or $isChange = 'delete') and cXML/Header/To/UserAgent = 'Copy'">
			...
			<DataArea>
				...
				<PurchaseOrder>
					<PurchaseOrderHeader>
						<DocumentReference>
							...
							<OperationReference>
								<Status>
									<Code name="OrderType">
										<xsl:value-of
											select="cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'orderType']"
										/>
									</Code>
								</Status>
								<Status>
									<Code name="Class">
										<xsl:choose>
											...
											<xsl:when
												...
												>CN</xsl:when>
											<xsl:when
												test="cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'orderType'] = 'Z4UB'"
												>RL</xsl:when>
										...
										<UserArea>
											<Status>
												...
												<Description type="TextDetail">
													<xsl:value-of
														select="cXML/Request/OrderRequest/OrderRequestHeader/Comments/text()"/>
												</Description>
											</Status>
										</UserArea>
									</Specification>
						...
						<Party role="SAPVendor">
							<xsl:call-template name="CreateParty">
								<xsl:with-param name="PartyInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'supplierCorporate']"/>
								<xsl:with-param name="IDInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'supplierCorporate']/@addressID"
								/>
							</xsl:call-template>
						</Party>
						<CustomerParty>
							<xsl:call-template name="CreateParty">
								<xsl:with-param name="PartyInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'soldTo']"/>
								<xsl:with-param name="IDInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'soldTo']/@addressID"
								/>
							</xsl:call-template>
						</CustomerParty>
						<SupplierParty>
						...
						<xsl:if
							test="cXML/Request/OrderRequest/OrderRequestHeader/ShipTo and cXML/Request/OrderRequest/OrderRequestHeader/ShipTo != ''">
							<ShipToParty>
								<xsl:call-template name="CreateParty">
									<xsl:with-param name="PartyInfo"
						...
						<BillToParty>
							<xsl:call-template name="CreateParty">
								...
								<xsl:with-param name="IDInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/BillTo/Address/@addressID"/>
							</xsl:call-template>
						</BillToParty>
						<CarrierParty>
							<xsl:call-template name="CreateParty">
								<xsl:with-param name="PartyInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'cargoDelivery']"/>
								<xsl:with-param name="IDInfo"
									select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'cargoDelivery']/@addressID"
								/>
							</xsl:call-template>
						</CarrierParty>
						...
						<xsl:if
							test="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'purchasingAgent']">
							<BuyerParty>
								<xsl:call-template name="CreateParty">
									<xsl:with-param name="PartyInfo"
										select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'purchasingAgent']"/>
									<xsl:with-param name="IDInfo"
										select="cXML/Request/OrderRequest/OrderRequestHeader/Contact[@role = 'supplierCorporate']/@addressID"
									/>
								</xsl:call-template>
						...
						<xsl:for-each
							select="cXML/Request/OrderRequest/OrderRequestHeader/OrderRequestHeaderIndustry/ReferenceDocumentInfo">
							<SalesOrderReference>
								<DocumentID>
								...
								<xsl:if
									test="Extrinsic[@name = 'incoTerm'] != '' or Extrinsic[@name = 'incoTermDesc'] != ''">
									<UserArea>
										<IncotermsCode>
