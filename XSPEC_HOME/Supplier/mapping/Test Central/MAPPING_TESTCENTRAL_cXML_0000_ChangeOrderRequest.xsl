...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0">
      ...
      <cXML>
         ...
         <Request>
            <OrderRequest>
               <OrderRequestHeader>
                  <xsl:copy-of
                     select="Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@*[name() != 'orderID' and name() != 'orderDate' and name() != 'type' and name() != 'orderVersion']"/>
                  ...
                  <xsl:if test="/Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@orderVersion!=''">                     
                     <xsl:attribute name="orderVersion">
                        <xsl:variable name="previousVersion" select="number(/Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@orderVersion)"/>
                        <xsl:value-of select="format-number($previousVersion+1, '#')"/>
                     </xsl:attribute>
                  </xsl:if>
         ...
         <Request>
            <OrderRequest>
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                     <OrderRequestHeader>
                        <xsl:copy-of select="Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@*[name() != 'orderID' and name() != 'orderDate' and name() != 'type' and name() != 'orderVersion']"/>
                        <xsl:attribute name="orderID">
                           <xsl:value-of select="$anDocumentID"/>
                        </xsl:attribute>
                        <xsl:attribute name="orderDate">
                  ...
                  <xsl:otherwise>
                     ...
                     <xsl:for-each select="/Combined/Payload/cXML/Request/OrderRequest/ItemOut">
                        <xsl:call-template name="generateOrderItem">
                           <xsl:with-param name="baseItem" select="."></xsl:with-param>
                           <xsl:with-param name="lineNum" select="@lineNumber"></xsl:with-param>
                           <xsl:with-param name="itemCharacteristics" select="/Combined/Characteristics/Details/Detail"></xsl:with-param>
                        </xsl:call-template>
                     </xsl:for-each>
                  </xsl:otherwise>
               </xsl:choose>
   ...
   <xsl:template name="ReplaceItemCharacteristics">
      ...
      <xsl:choose>
         <xsl:when
            test="/Combined/Payload/cXML/Request/OrderRequest/ItemOut[@lineNumber = $characteristicDetail/Record[FieldName = 'LineItemNumber']/FieldValue]">
            <xsl:call-template name="generateOrderItem">
               <xsl:with-param name="baseItem"
                  select="/Combined/Payload/cXML/Request/OrderRequest/ItemOut[@lineNumber = $chItemNumber]"/>
   ...
   <xsl:template name="ReplaceHeaderCharacteristics">
      ...
      <OrderRequestHeader>
         <xsl:copy-of select="/Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@*[name() != 'orderID' and name() != 'orderDate' and name() != 'type' and name() !='effectiveDate' and name()!='expirationDate' and name()!='orderVersion']"/>
         <xsl:attribute name="orderID">
            <xsl:value-of select="$anDocumentID"/>
         </xsl:attribute>
         <xsl:attribute name="orderDate">
         ...
         <xsl:attribute name="type">
            <xsl:choose>
               <xsl:when
                  test="/Combined/Characteristics/Header/Record[FieldName = 'Order Operation']/FieldValue = 'delete'">
         ...
         <xsl:if test="/Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@orderVersion!=''">
            <xsl:attribute name="orderVersion">
               <xsl:variable name="previousVersion" select="number(/Combined/Payload/cXML/Request/OrderRequest/OrderRequestHeader/@orderVersion)"/>
               <xsl:choose>
                  <xsl:when test="/Combined/Characteristics/Header/Record[FieldName = 'Order Operation']/FieldValue = 'delete'">
