      ...
      <M_ORDERS>
        ...
        <S_BGM>
          <C_C002>
            <D_1001>
              <xsl:choose>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'orderType'] = 'dropship'"
                  >220</xsl:when>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'orderType'] = 'rushorder'"
                  >224</xsl:when>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/@orderType = 'regular'">220</xsl:when>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/@orderType = 'blanket'">221</xsl:when>
              </xsl:choose>
            </D_1001>
            <D_1000>
              <xsl:choose>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'orderType'] = 'dropship'"
                  >Dropship Order</xsl:when>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/Extrinsic[@name = 'orderType'] = 'rushorder'"
                  >Rush Order</xsl:when>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/@orderType = 'regular'">Purchase Order</xsl:when>
                <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/@orderType = 'blanket'">Blanket Order</xsl:when>
              </xsl:choose>
            </D_1000>
          </C_C002>
          <C_C106>
          ...
          <D_1225>
            <xsl:choose>
              <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/@type = 'new'">9</xsl:when>
              <xsl:when test="cXML/Request/OrderRequest/OrderRequestHeader/@type = 'update'">5</xsl:when>
              <!-- 08172018 CG: Code for "update" added. Delete code is not allowed for ORDERS. -->
            </xsl:choose>
          </D_1225>
        </S_BGM>
        ...
        <xsl:if
          ...
          <G_SG2>
            <xsl:call-template name="createNAD">
              ...
              <xsl:with-param name="refGrupNum" select="'G_SG3'"/>
            </xsl:call-template>
          ...
          <G_SG8>
            ...
            <xsl:if test="Discount/DiscountAmount/Money != ''">
              <xsl:call-template name="createMOA">
                <xsl:with-param name="grpNum" select="'G_SG9'"/>
        ...
        <xsl:if
          ...
          <xsl:for-each select="cXML/Request/OrderRequest/OrderRequestHeader/TermsOfDelivery">
            ...
            <xsl:if
              ...
              <G_SG12>
                <S_TOD>
                  <xsl:if test="$Lookup/Lookups/TermsOfDeliveryCodes/TermsOfDeliveryCode[CXMLCode = $TODCode]/EANCOMCode != ''">
                    <D_4055>
                      <xsl:value-of
          ...
          <xsl:if test="normalize-space(ModificationDetail/@name) != ''">
            <G_SG19>
              <xsl:call-template name="mapALCFromModification">
                <xsl:with-param name="deductionGrpNum" select="'G_SG21'"/>
                <xsl:with-param name="moneyGrpNum" select="'G_SG22'"/>
              </xsl:call-template>
            </G_SG19>
          </xsl:if>
        </xsl:for-each>
        ...
        <xsl:for-each select="/cXML/Request/OrderRequest/ItemOut">
          <G_SG28>
            ...
            <!-- ItemOut > Packaging -->
            <!-- ItemOut taxpoint date -->
            <xsl:for-each select="Tax/TaxDetail[@isTriangularTransaction = 'yes' and (@taxPointDate != '' or @paymentDate != '')]">
              <G_SG33>
                <S_RFF>
                  <C_C506>
                    <D_1153>VA</D_1153>
