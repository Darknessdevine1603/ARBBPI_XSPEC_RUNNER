	...
	<xsl:template match="cXML/Request">
		<ChangeOrder>
			<ChangeOrderHeader>
				...
				<ChangeOrderSequence>
					<xsl:choose>
						<xsl:when test="OrderRequest/OrderRequestHeader/@orderVersion != ''">
							<xsl:choose>
								<xsl:when test="OrderRequest/OrderRequestHeader/@type = 'delete'">
									<xsl:value-of select="number(OrderRequest/OrderRequestHeader/@orderVersion) + 1"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="number(OrderRequest/OrderRequestHeader/@orderVersion)"/>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:when>
						<xsl:otherwise/>
				...
				<ListOfReferenceCoded>
					<xsl:call-template name="createOtherReferences">
						<xsl:with-param name="refCode">payloadID</xsl:with-param>
				...
				<Purpose>
					<PurposeCoded>
						<xsl:choose>
							<xsl:when test="OrderRequest/OrderRequestHeader/@type = 'delete'">Cancellation</xsl:when>
							<xsl:otherwise>Change</xsl:otherwise>
						</xsl:choose>
					</PurposeCoded>
				</Purpose>
				<ChangeType>
					<ChangeTypeCoded>ReplaceAllValues</ChangeTypeCoded>
				</ChangeType>
				<OrderType>
					<OrderTypeCoded>
						<xsl:choose>
					...
					<OrderHeader>
						...
						<OrderReferences>
							<xsl:choose>
								...
								<xsl:when test="OrderRequest/OrderRequestHeader/ShipTo/Address/@addressID and OrderRequest/OrderRequestHeader/ShipTo/Address/@addressID != ''">
									<AccountCode>
										<Reference>
											<RefNum>
						...
						<OrderType>
							<OrderTypeCoded>
								<xsl:choose>
									<xsl:when test="OrderRequest/OrderRequestHeader/@orderType = 'release'">ReleaseOrDeliveryOrder</xsl:when>
									<xsl:when test="OrderRequest/OrderRequestHeader/@orderType = 'blanket'">BlanketOrder</xsl:when>
									<!-- CC-018913 : Mark as servicer order -->
									<xsl:when test="OrderRequest/ItemOut/SpendDetail">SupplyOrServiceOrder</xsl:when>
									<xsl:otherwise>Order</xsl:otherwise>
								</xsl:choose>
							</OrderTypeCoded>
						...
						<!--xsl:if test="OrderRequest/OrderRequestHeader/Tax"-->
							...
							<OrderPaymentInstructions>
								<PaymentInstructions>
									<PaymentTerms>
										...
										<xsl:if test="OrderRequest/OrderRequestHeader/Extrinsic[@name = 'PaymentTermsNote'] != ''">
											<PaymentTermsNote>
												<xsl:value-of select="OrderRequest/OrderRequestHeader/Extrinsic[@name = 'PaymentTermsNote']"/>
											</PaymentTermsNote>
										</xsl:if>
									</PaymentTerms>
									<PaymentMethod>
					...
					<xsl:for-each select="OrderRequest/ItemOut">
						<ChangeOrderItemDetail>
							<ItemDetailChangeCoded>
								<xsl:choose>
									<xsl:when test="//Request/OrderRequest/OrderRequestHeader/@type = 'delete'">Cancelled</xsl:when>
									<xsl:otherwise>ReplaceAllValues</xsl:otherwise>
								</xsl:choose>
							</ItemDetailChangeCoded>
