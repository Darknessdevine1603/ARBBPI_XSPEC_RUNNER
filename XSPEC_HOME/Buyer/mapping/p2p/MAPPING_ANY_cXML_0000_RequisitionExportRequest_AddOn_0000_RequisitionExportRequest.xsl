...
<!--        <xsl:include href="FORMAT_Apps_0000_AddOn_0000.xsl"/>-->
    ...
    <!--This XSLT handles input from any of the 3 transaction - BudgetCheck,RTBC create(PR create) and RTBC change(PR change)-->
    <xsl:template
        match="RequisitionRealTimeBudgetExportRequest | RequisitionBudgetCheckExportRequest | RequisitionRevertBudgetToEarlierVersionExportRequest">
            ...
            <PRHEADERREQ>
                ...
                <PR_TYPE>
                    ...
                    <xsl:call-template name="FillDocType">
                        <xsl:with-param name="p_doctype" select="$anXsltDoctype"/>
                                        ...
                                        <xsl:choose>
                                            <xsl:when
                                                ...
                                                <!--Begin of IG-16774: If the user has entered the Unit Conversion in P2P UI then multiply price * conv. factor. 
                                                    Otherwise, map the price as it is. Before this fix we were sending wrongly the total calculated price to ERP based on the formula  ( Amount
