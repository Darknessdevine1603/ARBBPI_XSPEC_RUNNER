...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:n1="http://sap.com/xi/ARBCIG2"
...
<!--	<xsl:include href="../../../common/FORMAT_AddOn_0000_cXML_0000.xsl"/> -->
	...
	<!--	End of IG-37151-->    
	<xsl:template match="/n0:PurchasingContractERPConfirmation">
		<Combined>
			<Payload>
				<cXML>
					...
					<Header>
						...
						<To>
							...
							<Credential>
								...
								<Identity>
									<xsl:value-of select="PurchasingContract/SellerParty/InternalID"
									/>
								</Identity>
							</Credential>
							</To>
					...
					<Request>
						<ContractStatusUpdateRequest>
							...
							<ContractStatus>
								<xsl:choose>
									<xsl:when test="PurchasingContract/n1:ActionCode = '01'">
									...
									<xsl:when test="PurchasingContract/n1:ActionCode = '02'">
										<xsl:attribute name="type">
									...
									<xsl:when test="PurchasingContract/n1:ActionCode = '03'">
										<xsl:attribute name="type">
								...
								<ContractIDInfo>
									<xsl:attribute name="contractID">
										<xsl:value-of select="PurchasingContract/n1:AribaQuoteID" xmlns:n1="http://sap.com/xi/ARBCIG2"/>
									</xsl:attribute>
								...
								<xsl:for-each select="PurchasingContract/Item">
									<ContractItemStatus>
										<ItemStatus>
											<xsl:choose>
												<xsl:when test="n1:ActionCode = '01'">
													<xsl:attribute name="type">
												...
												<xsl:when test="n1:ActionCode = '02'">
													<xsl:attribute name="type">
												...
												<xsl:when test="n1:ActionCode = '03'">
													<xsl:attribute name="type">
											...
											<xsl:if test="exists(HierarchyRelationship)">
												<xsl:attribute name="parentLineNumber">
													<xsl:value-of
														select="HierarchyRelationship/ParentItemID"/>
												</xsl:attribute>
											</xsl:if>
											
											<ReferenceDocumentInfo>
												<xsl:attribute name="lineNumber">
													<xsl:value-of select="n1:AribaContractItemID"/>
												</xsl:attribute>
							...
							<Extrinsic name="SourcingEventId">
								<xsl:value-of select="PurchasingContract/n1:AribaContractID"/>
							</Extrinsic>
